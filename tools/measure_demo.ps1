...
[CmdletBinding()]
param([Parameter(Mandatory)][string]$Demo,[Parameter(Mandatory)][string]$Project,[ValidateSet('default','heldout','stress')][string]$Profile='default')
...
$env:DEMO_PROFILE=$Profile
$monitor=Start-Job -ArgumentList $Project,$report,$signal -ScriptBlock {
    param($projectName,$reportPath,$signalPath)
    while (Test-Path -LiteralPath $signalPath) {
        $ids=@(& docker ps --filter "label=com.docker.compose.project=$projectName" --format '{{.ID}}')
        if ($ids.Count) {
            $stamp=[DateTimeOffset]::UtcNow.ToString('o')
            foreach ($line in (& docker stats --no-stream --format '{{json .}}' @ids 2>$null)) {
                try { @{utc=$stamp;stats=($line | ConvertFrom-Json)} | ConvertTo-Json -Compress | Add-Content -LiteralPath (Join-Path $reportPath 'samples.jsonl') -Encoding utf8 } catch { }
            }
        }
