...
cd "$(dirname "$0")"
action=${1:-test}
project=${2:-meetings-wave2}
case "$action" in test|cloud|stop) ;; *) echo 'Use test, cloud, or stop' >&2; exit 2 ;; esac
case "$project" in meetings-?*) ;; *) echo 'Use an isolated meetings- project name' >&2; exit 2 ;; esac
...
local_report="../../artifacts/meeting-commitments/$action/$run"
...
sh ../../tools/demo_preflight.sh meeting-commitments "${DEMO_PROFILE:-default}"
compose build tests
docker image inspect munarium-meetings-runner:local --format '{{.Id}} {{.Size}}' > "$local_report/runner-image.txt"
compose run --rm --no-deps -e "MEETINGS_REPORT_DIR=$report/unit" unit unit
...
if [ "$action" = cloud ]; then
  ...
  for provider in openai anthropic openrouter; do
    if compose run --rm --no-deps bootstrap bootstrap "$provider" "$report/$provider/bootstrap"; then
      compose run --rm --no-deps -e "MEETINGS_REPORT_DIR=$report/$provider" tests "cloud-$provider" || failed=1
    else failed=1; echo "$provider bootstrap failed; continuing" >&2; fi
  done
  echo "Reports: artifacts/meeting-commitments/cloud/$run"
  exit "$failed"
fi
compose run --rm --no-deps bootstrap
compose run --rm --no-deps -e "MEETINGS_REPORT_DIR=$report/controlled" tests controlled
compose restart server
compose run --rm --no-deps -e "MEETINGS_REPORT_DIR=$report/restarted" tests restarted
compose run --rm --no-deps -e "MEETINGS_REPORT_DIR=$report/sdk" tests qualify
compose run --rm --no-deps app render "$report/controlled/case-001/status.txt" "$report/application.png"
echo "Reports: artifacts/meeting-commitments/test/$run"
