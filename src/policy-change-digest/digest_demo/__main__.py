...
def main():
    ...
    parser.add_argument(
        ...
        help="Fixture generation profile",
    )
    ...
    if args.command == "generate":
        ...
        bootstrap(args.inputs, Path("/credentials"), args.work, args.provider, model, True)
        ...
        print((args.work / "digest.md").read_text())
        if packet["analysis_status"] != "validated_draft":
            raise SystemExit(2)
    elif args.command == "usage":
        from .server import client

        with client(os.environ["MUNARIUM_MGMT_TOKEN"], "digest-reporter") as api:
            save(args.work / "usage.json", api.reports.usage().model_dump(mode="json"))
    else:
        from .server import ready

        env = dict(os.environ, DIGEST_REPORT_DIR=str(args.work))
        if args.command == "unit":
            subprocess.run(["ruff", "check", "digest_demo", "tests"], check=True)
            subprocess.run(["ruff", "format", "--check", "digest_demo", "tests"], check=True)
            ...
            if args.command == "cloud":
                ...
                env["DIGEST_CLOUD_PROVIDER"] = config["provider"]
