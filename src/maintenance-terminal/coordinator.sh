...
set -eu
action=${1:-controlled}
report=${MAINTENANCE_REPORT_DIR:?Set a fresh report directory}
mkdir -p "$report"
app=/opt/target/debug/maintenance-terminal
case "$action" in
  unit)
    cargo fmt -p maintenance-terminal -- --check > "$report/format.log" 2>&1
