...
cd "$(dirname "$0")"
action=${1:-test}
project=${2:-maintenance-wave1}
case "$action" in test|cloud|stop) ;; *) echo 'Use test, cloud, or stop' >&2; exit 2 ;; esac
case "$project" in maintenance-?*) ;; *) echo 'Use an isolated maintenance- project name' >&2; exit 2 ;; esac
...
local_report="../../artifacts/maintenance-terminal/$action/$run"
...
sh ../../tools/demo_preflight.sh maintenance-terminal "${DEMO_PROFILE:-default}"
compose build tests
docker image inspect munarium-maintenance-runner:local --format '{{.Id}} {{.Size}}' > "$local_report/runner-image.txt"
compose run --rm --no-deps -e "MAINTENANCE_REPORT_DIR=$report/unit" unit unit
...
if [ "$action" = cloud ]; then
  ...
  for provider in openai anthropic openrouter; do
    if compose run --rm --no-deps bootstrap bootstrap "$provider" --approve; then
      compose run --rm --no-deps -e "MAINTENANCE_REPORT_DIR=$report/$provider" tests "cloud-$provider" || failed=1
    else failed=1; echo "$provider bootstrap failed; continuing" >&2; fi
  done
  echo "Reports: artifacts/maintenance-terminal/cloud/$run"
  exit "$failed"
fi
compose run --rm --no-deps bootstrap
compose run --rm --no-deps -e "MAINTENANCE_REPORT_DIR=$report/controlled" tests controlled
compose restart server
compose run --rm --no-deps -e "MAINTENANCE_REPORT_DIR=$report/restarted" tests restarted
compose run --rm --no-deps -e "MAINTENANCE_REPORT_DIR=$report/sdk" tests qualify
compose run --rm --no-deps --entrypoint python3 app /app/support.py render "$report/controlled/case-001/answer.txt" "$report/application.png"
echo "Reports: artifacts/maintenance-terminal/test/$run"
