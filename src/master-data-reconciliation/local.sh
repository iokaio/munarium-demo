...
cd "$(dirname "$0")"
action=${1:-test}
project=${2:-reconcile-wave2}
case "$action" in test|cloud|stop) ;; *) echo 'Use test, cloud, or stop' >&2; exit 2 ;; esac
case "$project" in reconcile-?*) ;; *) echo 'Use an isolated reconcile- project name' >&2; exit 2 ;; esac
...
local_report="../../artifacts/master-data-reconciliation/$action/$run"
...
if [ "$action" = cloud ]; then
  ...
  for provider in openai anthropic openrouter; do
    if compose run --rm --no-deps bootstrap bootstrap "$provider" --approve; then
      compose run --rm --no-deps -e "RECONCILE_REPORT_DIR=$report/$provider" tests "cloud-$provider" || failed=1
    else failed=1; echo "$provider bootstrap failed; continuing" >&2; fi
  done
  echo "Reports: artifacts/master-data-reconciliation/cloud/$run"
  exit "$failed"
fi
...
echo "Reports: artifacts/master-data-reconciliation/test/$run"
