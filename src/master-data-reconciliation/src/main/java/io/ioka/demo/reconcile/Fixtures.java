...
public final class Fixtures {
    ...
    public static void generate(Path inputs, Path oracle) throws Exception {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            first.append("SUPPLIER ").append(number).append(',').append(fields[i]).append(',').append(a[i]).append('\n');
            second.append("supplier_").append(number).append(',').append(fields[i]).append(',').append(b[i]).append('\n');
            String recommendation = i < 6 ? "prefer_b" : i == 6 ? "retain_a" : "review_ambiguous";
            FilesUtil.write(inputs.resolve("documents/" + id + ".txt"), "Synthetic Lumen Stewardship, seed 6091, logical date 2026-09-11.\nPolicy revision: STEWARD-" + number + "-R1. Field: " + fields[i] + ". Recommendation: " + recommendation + ".\n" + (i < 6 ? "Export B reflects the approved directory review. A steward may approve a correction after inspecting both rows." : i == 6 ? "Supporting regional approval is missing. Retain export A pending evidence; do not infer a new tax region." : "Source ownership is ambiguous. Do not resolve the bank review state automatically; retain the current value pending a steward decision.") + "\nA ledger acceptance is a recorded governance outcome, not proof of factual correctness.\n");
            expected.put(id, Map.of("recommendation", recommendation, "approved", i < 6, "current", i < 6 ? b[i] : a[i], "prior", a[i], "revision", "STEWARD-" + number + "-R1"));
        }
        ...
        FilesUtil.save(inputs.resolve("manifest.json"), Map.of("seed",6091,"generator_version",1,"template_revision","stewardship-1","logical_date","2026-09-11","timezone","UTC","locale","ROOT","cases",8,"files",hashes));
    ...
    public static void verify(Path inputs) throws Exception {
        ...
        if (manifest.path("seed").asInt()!=6091 || manifest.path("generator_version").asInt()!=1 || manifest.path("files").size()!=10) throw new IllegalArgumentException("Unexpected fixture manifest");
