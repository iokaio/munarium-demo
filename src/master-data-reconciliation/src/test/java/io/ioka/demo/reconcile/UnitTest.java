...
class UnitTest {
    ...
    @Test void independentProcessesGenerateIdenticalInputsAndOracle() throws Exception {
        for(String name:new String[]{"a","b"}) {var child=new ProcessBuilder("/app/build/install/master-data-reconciliation/bin/master-data-reconciliation","generate",temporary.resolve(name).toString(),temporary.resolve(name+"-oracle").toString()).start();assertTrue(child.waitFor(20,TimeUnit.SECONDS));assertEquals(0,child.exitValue());}
        ...
        FilesUtil.write(Path.of(System.getenv("RECONCILE_REPORT_DIR"),"fixture-manifest.json"),Files.readString(temporary.resolve("a/manifest.json")));
    }
