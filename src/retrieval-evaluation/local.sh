...
set -eu
cd "$(dirname "$0")"
action=${1:-test}; project=${2:-bench-wave2}
case "$project" in bench-?*) ;; *) exit 2 ;; esac
...
report="/work/$action/$run"; local_report="../../artifacts/retrieval-evaluation/$action/$run"
...
sh ../../tools/demo_preflight.sh retrieval-evaluation "${DEMO_PROFILE:-default}"
compose build tests
docker image inspect munarium-bench-runner:local --format '{{.Id}} {{.Size}}' > "$local_report/runner.txt"
...
if [ "$action" = cloud ]; then
  ...
  echo "Reports: artifacts/retrieval-evaluation/cloud/$run"
  exit "$failed"
fi
...
compose run --rm --no-deps scorer score --work "$report/controlled/experiments"
compose run --rm --no-deps --entrypoint /usr/bin/python3 scorer /app/support.py plot "$report/controlled/experiments/metrics.json" "$report/application.png"
echo "Reports: artifacts/retrieval-evaluation/test/$run"
