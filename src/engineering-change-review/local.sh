...
cd "$(dirname "$0")"
action=${1:-test}
project=${2:-engineering-wave3}
case "$action" in test|cloud|stop) ;; *) echo 'Use test, cloud, or stop' >&2; exit 2 ;; esac
case "$project" in engineering-?*) ;; *) echo 'Use an isolated engineering- project name' >&2; exit 2 ;; esac
...
local_report="../../artifacts/engineering-change-review/$action/$run"
...
sh ../../tools/demo_preflight.sh engineering-change-review "${DEMO_PROFILE:-default}"
compose build tests
docker image inspect munarium-engineering-runner:local --format '{{.Id}} {{.Size}}' > "$local_report/runner-image.txt"
compose run --rm --no-deps -e "ENGINEERING_REPORT_DIR=$report/unit" unit unit
...
if [ "$action" = cloud ]; then
  ...
  for provider in openai anthropic openrouter; do
    if compose run --rm --no-deps bootstrap bootstrap "$provider" --approve; then
      compose run --rm --no-deps -e "ENGINEERING_REPORT_DIR=$report/$provider" tests "cloud-$provider" || failed=1
    else failed=1; echo "$provider bootstrap failed; continuing" >&2; fi
  done
  echo "Reports: artifacts/engineering-change-review/cloud/$run"
  exit "$failed"
fi
compose run --rm --no-deps bootstrap
compose run --rm --no-deps -e "ENGINEERING_REPORT_DIR=$report/controlled" tests controlled
compose restart server
compose run --rm --no-deps -e "ENGINEERING_REPORT_DIR=$report/restarted" tests restarted
compose run --rm --no-deps -e "ENGINEERING_REPORT_DIR=$report/sdk" tests qualify
compose run --rm --no-deps --entrypoint python3 app /app/support.py render "$report/controlled/case-001/review.md" "$report/application.png"
echo "Reports: artifacts/engineering-change-review/test/$run"
