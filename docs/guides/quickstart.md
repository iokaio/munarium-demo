...
Open <http://localhost:5310/support>. Initialization generates six independent random secrets in ignored `.env`, retaining existing values on rerun. Starting creates PostgreSQL, Server 1.1.1, Ollama, and the web app, then downloads the completion and embedding models. Loading verifies source hashes, applies the provider and shapes, binds runbook default models to your selected provider, uploads documents, and builds indexes. `--approve` authorizes cutover only for runs recorded by this checkout.
