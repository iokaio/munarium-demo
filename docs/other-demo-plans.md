...
The recommended first wave is an invoice exception batch, an employee policy desktop assistant, an order exception consumer, and a maintenance terminal. Together they give each Server client language a practical entry point. The invoice batch, employee policy desktop assistant, order exception consumer, and maintenance terminal are implemented references for repository layout and testing; the remaining applications are proposals. Later demos teach ingestion operations, governed memory, historical reconstruction, and evaluation. Recorded test results describe synthetic workloads, not measured business outcomes.
