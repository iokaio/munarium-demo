...
The first wave comprises an invoice exception batch, an employee policy desktop assistant, an order exception consumer, and a maintenance terminal. Together they give each Server client language a practical entry point. All thirteen planned applications are implemented, including the Matrix-backed inventory briefing, and are cataloged in the [repository README](../README.md#additional-demo-applications). Each walkthrough records its own qualification evidence and remaining host-platform limits. Later demos teach historical reconstruction and evaluation. Recorded test results describe synthetic workloads, not measured business outcomes. Applications and their qualification updates were completed and committed one at a time. Do not push these commits; the developer will review and push the changes.

**Completion status, 2026-09-11.** All thirteen demos now have default, separately seeded held-out, and larger stress fixtures; native-language qualification; fresh-checkout tests with empty project state; and recorded resource measurements. Every AI demo has a fresh passing OpenAI/Anthropic/OpenRouter run, with OpenRouter kept to two independent cases and paced requests. The evaluation bench repeats its eight cases twice. All walkthroughs begin with a business case. The employee assistant additionally passed native Windows x64 input, source inspection, file-picker export and identity-state clearing.

**Remaining required platform qualification.** Windows Docker Desktop with Linux/AMD64 containers, PowerShell, Git Bash and Ubuntu WSL are qualified as recorded in each walkthrough. Native Linux and macOS hosts, native ARM64 and Apple Silicon AMD64 emulation remain pending because those environments were not available. The employee desktop also needs its native Linux/macOS checks. Follow the [host qualification checklist](demo-qualification.md#host-and-architecture-qualification) and retain those results separately. Matrix's pinned upstream build selects AMD64. Optional extensions such as the inventory Java port are not required for the implemented portfolio.
...
**Implementation status.** The [invoice exception demo](demos/invoice-exception/README.md) implements the batch workflow, separately seeded default and held-out corpora, and a 182-invoice stress profile. All three profiles passed 32 application checks and 175 Python SDK checks with four documented chronology skips. The final default profile also passed from a fresh checkout through Ubuntu WSL with empty state. The rebalanced cloud suite passed eight fresh cases with a 3/3/2 OpenAI/Anthropic/OpenRouter split and 60-second OpenRouter pacing. Its walkthrough records profile definitions, resource measurements, retained failures and remaining native-host limits.
...
**Implementation status.** The [employee policy assistant](demos/employee-policy-assistant/README.md) implements the C#/.NET 10 Avalonia desktop workflow with the official client. Default, independently seeded held-out, and 70-document stress profiles each passed 27 application tests and 82 SDK tests, with two documented chronology skips. The default also passed from a fresh checkout through Ubuntu WSL with empty state. Six fresh cloud cases passed, with two per online provider and paced OpenRouter requests. A native Windows x64 check passed launch, input, grounded answer/source inspection, native file-picker export, and identity-state clearing. The walkthrough records measurements, exact runs and remaining native Linux/macOS/ARM64 limits.
...
**Implementation status.** The [order exception triage demo](demos/order-exception-triage/README.md) implements the Java consumer with independent scoped sessions and durable inbox/outbox recovery. Default and independently seeded held-out profiles each passed 31 application checks; the 80-event stress profile passed 103. Each also passed 63 SDK tests with one documented chronology skip. Default passed from a fresh checkout through Ubuntu WSL with empty state. Eight fresh cloud cases passed with a 3/3/2 split and enforced 60-second OpenRouter pacing. The walkthrough records profile manifests, measured resources, retained failed runs and remaining native-host limits.
...
**Implementation status.** The [maintenance procedure terminal](demos/maintenance-terminal/README.md) implements the Rust retrieval-first TUI with exact asset/revision scopes and optional cited completion. Default and independently seeded held-out profiles each passed 25 application checks; the 80-case, 240-document stress profile passed 97. All profiles passed 79 SDK checks, with the upstream kernel-chronology coverage gap documented separately. The final source passed from a fresh checkout through Ubuntu WSL with empty state. Eight fresh cloud cases passed with a 3/3/2 split and 60-second OpenRouter pacing. The walkthrough records profile manifests, resource measurements and remaining native-host limits.
...
**Implementation status.** The [master-data reconciliation demo](demos/master-data-reconciliation/README.md) implements normalized exports, cited drafts, explicit steward review and governed ledger corrections. Default and independently seeded held-out profiles each passed 28 application checks; the 80-pair stress profile passed 100. Every profile includes the actual two-container head-conflict race and passed 63 SDK checks with one documented chronology skip. Final source passed from a fresh checkout through Ubuntu WSL with empty state. Eight fresh cloud cases passed with a 3/3/2 split and paced OpenRouter requests. The walkthrough records manifests, measurements and remaining native-host limits.
...
**Implementation status.** The [policy change digest](demos/policy-change-digest/README.md) separates before, after and checklist evidence, computes textual diffs and checkpoints cited candidate impacts. Default and independently seeded held-out profiles each passed 25 application checks; the 80-pair, 320-document stress profile passed 97. Every profile also passed 175 SDK checks with four documented chronology skips. Final source passed from a fresh checkout through Ubuntu WSL with empty state, and eight fresh online cases passed with the 3/3/2 allocation and paced OpenRouter calls. The walkthrough records manifests, measurements and remaining native-host limits.
