...
The sample must use UTF-8 dotenv text with one `KEY=value` assignment per line, comments on their own lines beginning with `#`, and no shell `export` prefix. Empty values mean the optional provider is unconfigured. Document single-quoting values that contain spaces, `#`, or `$` so the Compose environment-file parser treats them literally. Include at least the following sample content and extend it when adding another provider:
